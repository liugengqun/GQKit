...
@property (assign, nonatomic) BOOL fixedSize;
...
+ (instancetype)buttonWithTag: (SKTag *)tag {
    ...
    if (tag.attributedText) {
        ...
        [btn setBackgroundImage:[UIImage gq_imageWithColor:tag.selectBgColor] forState:UIControlStateSelected];
    ...
    if(tag.bgSelectImg != nil){
        [btn setBackgroundImage:tag.bgSelectImg forState:UIControlStateSelected];
    }
    ...
    if (!CGSizeEqualToSize(tag.fixedSize, CGSizeZero)) {
        btn.fixedSize = YES;
        btn.frame = CGRectMake(0, 0, tag.fixedSize.width, tag.fixedSize.height);
    }
