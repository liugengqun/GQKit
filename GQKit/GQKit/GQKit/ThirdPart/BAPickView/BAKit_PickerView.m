...
- (void)ba_layoutSubViews
{
    ...
    if (self.buttonPositionType == BAKit_PickerViewButtonPositionTypeNormal)
    {
        ...
        //liugengqun
        if (GQ_iPhoneLH) {
            self.pickView.frame = CGRectMake(min_x, min_y, min_w, min_h + 34);
        }
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    else if (self.pickerViewType == BAKit_PickerViewTypeMultipleArray)
    {
        ...
        if (self.MultipleArrayTypeBlock) {
            self.MultipleArrayTypeBlock(self.selectedArray);
        }
    }
    else if (self.pickerViewType == BAKit_PickerViewTypeDate)
    {
...
- (void)ba_pickViewShowAnimation
{
    ...
    switch (self.animationType) {
        ...
        {
            [self.bgView ba_animation_showFromPositionType:BAKit_ViewAnimationEnterDirectionTypeBottom duration:0.25f finishBlock:^{
                BAKit_StrongSelf
                self.isAnimating = NO;
            }];
        }
...
- (void)ba_pickViewHiddenAnimation
{
    ...
    switch (self.animationType) {
        ...
        case BAKit_PickerViewAnimationTypeBottom:
        {
            [self.bgView ba_animation_dismissFromPositionType:BAKit_ViewAnimationEnterDirectionTypeBottom duration:0.25f finishBlock:^{
                BAKit_StrongSelf
                self.bgView.alpha = 0.1f;
                self.isAnimating = NO;
                [self ba_removeSelf];
